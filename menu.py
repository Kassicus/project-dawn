...
class PlayerInventoryMenu():
    def __init__(self, x, y, bg_color, fg_color):
        ...
        self.menuS1BtnDict = {}    # Dictionary of buttons for menu section 1
        ...
        for s in range(1,self.menuRows+1):
            ...
            if s == 1:
                value.isActive = True
                self.activeButtonKey = self.menuS1BtnKey+str(s)
            value.screenNum = s
    ...
    def draw(self):
        ...
        self.menuOptionDict[self.mO]+" "+str(self.screenNum),
        self.textColor1,
        size=self.menuTextSize,
        style=freetype.STYLE_UNDERLINE #| freetype.STYLE_OBLIQUE,
        )
    ...
    def menuClick(self):
        """Handles anything to do with clicking in the menu
        """
        ...
        if pygame.mouse.get_pressed()[0]: # Check for the mouse left click
            ...
            for i in range(1,self.menuRows+1):
                if self.menuS1BtnDict[self.menuS1BtnKey+str(i)].isHovered and self.menuS1BtnDict[self.menuS1BtnKey+str(i)].isActive == False and self.activeButtonKey != self.menuS1BtnKey+str(i):
                    self.menuS1BtnDict[self.activeButtonKey].isActive = False
